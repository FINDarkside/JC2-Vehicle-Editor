...
public class Project {
    ...
    public Project(File file) throws IOException, InterruptedException, ParserConfigurationException, SAXException {
        ...
        if (file.getName().startsWith("lave")) {
            if (new File(Settings.currentPath + "\\Files\\Default vehicles\\Cars\\" + file.getName() + ".eez").exists()) {
                parse(FieldsDictionary.getCar());
            } else {
                parse(FieldsDictionary.getBike());
            }
        } else if (file.getName().contains("arve")) {
            if (new File(Settings.currentPath + "\\Files\\Default vehicles\\Helicopters\\" + file.getName() + ".eez").exists()) {
                parse(FieldsDictionary.getHelicopter());
            } else {
                parse(FieldsDictionary.getPlane());
            }
        } else {
            parse(FieldsDictionary.getBoat());
        }
    }
